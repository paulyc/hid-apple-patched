...
MODULE_PARM_DESC(capslockmode, "Caps Lock Mode, aka 'Pimp My Caps' "
    ...
    "1 = sparcstation-mode: Caps Lock key is <>, or 'Meta' but in the emacs sense, in lieu of Ctrl, "
    "for people who use EMACS; physical Caps Lock LED on the keyboard is off), ");
...
static unsigned int applespi_code_to_key(u8 code, int fn_pressed)
{
	...
	if (capslockmode)
		key = applespi_translate_caps_key(key);
	return key;
}
